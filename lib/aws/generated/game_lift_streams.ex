...
defmodule AWS.GameLiftStreams do
  ...
  Use Amazon GameLift Streams
  tools to upload and configure content for streaming, deploy and scale computing
  resources
  ...
  use the API through the
  Amazon Web Services SDK, the Command Line Interface (AWS CLI), or by making
  ...
  using `DisassociateApplications`.
  """
  ...
  application content files to an Amazon Simple Storage Service (Amazon S3)
  bucket. For more information, see **Getting Started** in the Amazon GameLift
  ...
  **On-demand**:
  ...
  status, you can start stream sessions by using `StartStreamSession`. To check
  the stream group's status, call `GetStreamGroup`.
  """
  ...
  session disconnects that a reconnection is allowed. If a client is disconnected
  from the
  ...
  application content files stored with Amazon GameLift Streams. However, this
  does not delete the
  original files that you uploaded to your Amazon S3 bucket; you can delete these
  any time
  after Amazon GameLift Streams creates an application, which is the only time
  Amazon GameLift Streams accesses your Amazon S3
  bucket.
  ...
  first delete the stream group by using `DeleteStreamGroup`.

    *
  The application is not linked to any stream groups. You must first unlink the
  stream group by using `DisassociateApplications`.

    *

  An application is not streaming in any ongoing stream session. You must wait
  until the client ends the stream session or call `TerminateStreamSession` to end
  the stream.
  ...
  deleting the stream group, call `ListStreamSessions` to check for streams in
  ...
  streams using this stream group. The disassociate action does not affect the
  ...
  application that you want to retrieve. If the operation is successful, it
  returns
  ...
  Specify the ID of the stream
  group that you want to retrieve. If the operation is successful, it returns
  properties
  ...
  Specify the Amazon Resource Name (ARN) of the
  stream session that you want to retrieve and its stream group ARN. If the
  operation is successful, it returns properties
  ...
  This operation returns applications in all statuses, in no
  ...
  This operation returns stream groups in all statuses, in no
  particular order. You can paginate the results as needed.
  """
  ...
  To list tags for a resource, specify
  ...
  *Amazon Web Services General Reference*
  ...
  stream group before starting a stream session, either when
  ...
  Use tags to organize Amazon Web Services resources for
  a range of purposes. You can assign tags to the following Amazon GameLift
  Streams resource types:
  ...
  *Amazon Web Services General Reference*
  ...
  When called, the stream session
  ...
  You
  can change the description, the set of locations, and the requested capacity of
  a stream group per location. If you want to change the stream class, create a
  new stream group.
  ...
  **On-demand**:
  ...
  and provide the new values. If
  the request is successful, Amazon GameLift Streams returns the complete updated
