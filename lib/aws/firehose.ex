...
defmodule AWS.Firehose do
  ...
  created, its status is `ACTIVE` and it now accepts data. If the delivery
  ...
  is optional. You can also invoke `StartDeliveryStreamEncryption` to turn on
  ...
  continue to accept records, but it doesn't make any guarantees with respect
  to delivering the data. Therefore, as a best practice, first stop any
  ...
  your delivery stream is created, call `DescribeDeliveryStream` to see
  ...
  stream is the `Status` property in `DeliveryStreamEncryptionConfiguration`.
  ...
  encryption status is `ENABLING`, but the data is not encrypted. It can take
  up to 5 seconds after the encryption status changes to `ENABLED` before all
  ...
  `CUSTOMER_MANAGED_CMK`, Kinesis Data Firehose creates a grant that enables
  ...
  you get `ENABLING_FAILED`, this only means that the attempt to change the
  CMK failed. In this case, encryption remains enabled with the old CMK.
  ...
  records written to the delivery stream are no longer subject to encryption.
  To find out whether a record or a batch of records was encrypted, check the
