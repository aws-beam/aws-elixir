...
defmodule AWS.ECS do
  ...
  they're in the `RUNNING` state. Tasks for services that *do* use a load balancer
  ...
  the desired count of a service, with an `UpdateService` operation. The default
  ...
  of four tasks and a minimum healthy percent of 50%, the scheduler might stop two
  ...
  of the desired number of tasks (rounded down to the nearest integer), and while
  ...
  tasks and a maximum percent value of 200%, the scheduler may start four new
  ...
  `RUNNING` state while the container instances are in the `DRAINING` state. If
  ...
  The cluster will transition to the `INACTIVE` state. Clusters with an `INACTIVE`
  status may remain discoverable in your account for a period of time. However,
  this behavior is subject to change in the future, so you should not rely on
  ...
  count is zero. If the service is actively maintaining tasks, you cannot delete
  ...
  name. You can also filter the results by attribute name and value, for example,
  ...
  all of the IAM users and roles for which no individual account setting has been
  specified are reset. For more information, see [Account Settings](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-account-settings.html)
  in the *Amazon Elastic Container Service Developer Guide*.
  ...
  Amazon ECS features such as resource tagging.
  ...
  If the attribute does not exist, it is created. If the attribute exists, its
  ...
  Tasks for services that do not use a load balancer are considered healthy if
  they are in the `RUNNING` state. Tasks for services that use a load balancer are
  considered healthy if they are in the `RUNNING` state and the container instance
  ...
  platform version, or task definition need to be updated, you should create a new
