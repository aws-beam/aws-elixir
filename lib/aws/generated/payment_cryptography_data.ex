...
defmodule AWS.PaymentCryptographyData do
  ...
  **Cross-account use**: This operation supports cross-account use when the key
  ...
  Amazon Web Services Payment Cryptography supports the following encryption key
  schemes:

    * **DUKPT**: `TDES` and `AES` algorithms. To encrypt using DUKPT, a
  DUKPT key must already exist within your account with `KeyModesOfUse` set to
  `DeriveKey` or a new DUKPT can be generated by calling
  [CreateKey](https://docs.aws.amazon.com/payment-cryptography/latest/APIReference/API_CreateKey.html).     * **Symmetric**: `TDES` and `AES` algorithms.

    * **Asymmetric**: `RSA`. You can use an `RSA` key on either the
  incoming or the outgoing side, paired with a symmetric key on the other side.
