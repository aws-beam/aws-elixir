...
defmodule AWS.Firehose do
  ...
  providers using `PutRecord` or `PutRecordBatch`, or it can be configured to use
  an existing Kinesis stream as its source. To specify a Kinesis data stream as
  input, set the `DeliveryStreamType` parameter to `KinesisStreamAsSource`, and
  ...
  satisfied first. `BufferingHints` is a hint, so there are some cases where the
  ...
  intermediate location. Firehose first delivers data to Amazon S3 and then uses
  ...
  provide exclusively with Firehose, and that the permissions for the account are
  ...
  have permissions that allow the service to deliver the data. For more
  ...
  `ACTIVE`, `DELETING`, `CREATING_FAILED`, or `DELETING_FAILED`. You can't delete
  a delivery stream that is in the `CREATING` state. To check the state of a
  ...
  increase, see [Amazon Firehose Limits](https://docs.aws.amazon.com/firehose/latest/dev/limits.html).
  ...
  to `ENABLED`. The encryption status of a delivery stream is the `Status`
  property in `DeliveryStreamEncryptionConfiguration`. If the operation fails, the
